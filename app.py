...
# NAVIGABILITÀ TRA REPARTI TRAMITE SCHEDE (TAB)
...
with tab_1d:
    ...
    if st.button("🚀 AVVIA OTTIMIZZAZIONE BARRE", type="primary"):
        ...
        # Simulazione algoritmo di taglio stabile (First Fit Decreasing) con recupero sfrido utile
        tagli_barre = []
        barra_corrente = []
        spazio_rimasto = 6000
        
        for pezzo in reqs:
            if pezzo <= spazio_rimasto:
                barra_corrente.append(pezzo)
                spazio_rimasto -= pezzo
            else:
                if barra_corrente:
                    tagli_barre.append({"Tagli (mm)": str(barra_corrente), "Sfrido (mm)": spazio_rimasto})
                barra_corrente = [pezzo]
                spazio_rimasto = 6000 - pezzo
        if barra_corrente:
            tagli_barre.append({"Tagli (mm)": str(barra_corrente), "Sfrido (mm)": spazio_rimasto})
...
with tab_2d:
    ...
    # PARAMETRI MACCHINA CNC
    st.sidebar.markdown("<h3 style='color:#FF5722;'>📊 Formato Lastra</h3>", unsafe_allow_html=True)
    W_lamiera = st.sidebar.number_input("Lunghezza Lastra X (mm)", value=3000, step=100)
    ...
    distanza_sicurezza = st.sidebar.number_input("Distanza Sicurezza (mm)", value=4.0, step=1.0)
    ...
    # CALCOLO E DISEGNO DEL NESTING 2D AD INCASTRO
    if st.button("🚀 AVVIA ELABORAZIONE NESTING LAMIERE", type="primary"):
        if not lista_particolari: st.error("Nessun file DXF caricato nello slot.")
        else:
            coda = []
            ...
            for item in coda:
                ...
                # RISOLTO CRASH CHAT: Generazione dinamica degli angoli (0, 45, 90, 135, 180, 225, 270, 315)
            ...
            # MAGGAZINO RECUPERO SFRIDI LASTRE (COME NEI PROFILATI 1D)
            if piazzati:
